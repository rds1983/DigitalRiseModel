...
namespace DigitalRiseModel.Primitives
{
	partial class MeshPrimitives
	{
		...
		public static DrMeshPart CreateDiscMeshPart(GraphicsDevice graphicsDevice, float radius = 0.5f, float sectorAngle = 2 * MathF.PI, int tessellation = 16, float uScale = 1.0f, float vScale = 1.0f, bool toLeftHanded = false)
		{
			if (tessellation < 4)
				tessellation = 4;
		...
		/// <param name="toLeftHanded">if set to <c>true</c> vertices and indices will be transformed to left handed. Default is false.</param>
		/// <returns>A cone.</returns>
		public static DrMesh CreateDiscMesh(GraphicsDevice graphicsDevice, float radius = 0.5f, float sectorAngle = 2 * MathF.PI, int tessellation = 16, float uScale = 1.0f, float vScale = 1.0f, bool toLeftHanded = false)
		{
