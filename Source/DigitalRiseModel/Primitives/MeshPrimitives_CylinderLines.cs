...
namespace DigitalRiseModel.Primitives
{
	partial class MeshPrimitives
	{
		...
		/// <returns>A new <see cref="DrSubmesh"/> that represents a cylinder line list.</returns>
		/// <remarks>
		/// If the returned <see cref="DrSubmesh"/> is not going to be modified, then it is better
		/// to call <see cref="GetCylinderLines"/> to retrieve a shared <see cref="DrSubmesh"/> instance.
		/// </remarks>
		/// <exception cref="ArgumentOutOfRangeException">
		/// <paramref name="numberOfSegments"/> is less than or equal to 2.
		/// </exception>
		public static DrSubmesh CreateCylinderLinesSubmesh(GraphicsDevice graphicsDevice, int numberOfSegments = 32)
		{
