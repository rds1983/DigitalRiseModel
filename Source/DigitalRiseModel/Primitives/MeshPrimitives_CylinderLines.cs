...
namespace DigitalRiseModel.Primitives
{
	partial class MeshPrimitives
	{
		...
		/// <returns>A new <see cref="DrMeshPart"/> that represents a cylinder line list.</returns>
		/// <remarks>
		/// If the returned <see cref="DrMeshPart"/> is not going to be modified, then it is better
		/// to call <see cref="GetCylinderLines"/> to retrieve a shared <see cref="DrMeshPart"/> instance.
		/// </remarks>
		/// <exception cref="ArgumentOutOfRangeException">
		/// <paramref name="numberOfSegments"/> is less than or equal to 2.
		/// </exception>
		public static DrMeshPart CreateCylinderLinesMeshPart(GraphicsDevice graphicsDevice, int numberOfSegments = 32)
		{
